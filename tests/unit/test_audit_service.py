"""End-to-end tests for the audit pipeline (router -> fakes -> engine -> persistence).

The classifier and extractor are scripted fakes, so these exercise the real router, the
real gap engine, and real SQLite persistence with no network and no API keys.
"""
...
def _complete_packet() -> tuple[list[tuple[str, bytes]], FakeClassifier, FakeExtractor]:
    files = [
    ...
    classifier = FakeClassifier(
        {
            "invoice.txt": Classification("commercial_invoice", 0.97),
            "packing.txt": Classification("packing_list", 0.96),
            "bol.txt": Classification("bill_of_lading", 0.95),
            "origin.txt": Classification("certificate_of_origin", 0.94),
        }
    )
    extractor = FakeExtractor(
        {
            ...
            "origin.txt": _fields(country_of_origin="China"),
        }
    )
    return files, classifier, extractor
...
def test_complete_packet_is_clear_and_persisted(
    ...
    files, classifier, extractor = _complete_packet()
    ...
    # Packet + its four documents were persisted, scoped to the session.
...
def test_deficient_field_surfaces_as_gap(
    ...
    files, _classifier, extractor = _complete_packet()
    classifier = FakeClassifier(
        {
            "invoice.txt": Classification("commercial_invoice", 0.97),
            "packing.txt": Classification("packing_list", 0.96),
    ...
    extractor._by_doc_id["invoice.txt"]["hts_code"] = ExtractedField("hts_code", "12", 0.95)
    service = make_audit_service(classifier=classifier, extractor=extractor)
...
def test_unrecognized_file_needs_review(
    ...
    files, _c, extractor = _complete_packet()
    files.append(("mystery.txt", _TEXT.encode()))
    classifier = FakeClassifier(
        {
            "invoice.txt": Classification("commercial_invoice", 0.97),
            "packing.txt": Classification("packing_list", 0.96),
            "bol.txt": Classification("bill_of_lading", 0.95),
            "origin.txt": Classification("certificate_of_origin", 0.94),
            # mystery.txt intentionally unmapped -> classified as None.
        }
    )
    service = make_audit_service(classifier=classifier, extractor=extractor)
    ...
    classifier = FakeClassifier(
        {
            "invoice.txt": Classification("commercial_invoice", 0.97),
            "packing.txt": Classification("packing_list", 0.96),
            "bol.txt": Classification("bill_of_lading", 0.20),  # unsure
            "origin.txt": Classification("certificate_of_origin", 0.94),
        }
    )
    service = make_audit_service(classifier=classifier, extractor=extractor)
...
def test_too_many_files_rejected(
    ...
    files, classifier, extractor = _complete_packet()
    service = make_audit_service(classifier=classifier, extractor=extractor, max_files=2)
...
def test_empty_packet_rejected(
    ...
    _f, classifier, extractor = _complete_packet()
    service = make_audit_service(classifier=classifier, extractor=extractor)
...
def test_sessions_are_isolated(
    ...
    files, classifier, extractor = _complete_packet()
    svc_a = make_audit_service("session_a", classifier=classifier, extractor=extractor)
    result_a = svc_a.audit_packet(files)

    # A different session cannot see session_a's packet.
    with session_factory() as db:
